...
contract DeployScroll is Script {
    ...
    // Facet addresses
    address public rewardDistributorFacet;
    ...
    function setUp() public {
        ...
        if (
            keccak256(abi.encodePacked(deploymentTarget)) ==
        ...
        if (
            ...
            require(
                ...
                "Must be on Scroll Sepolia testnet"
            );
        }
    ...
    function deployDiamondWithFacets() internal {
        ...
        treasuryProxy = address(
            new ERC1967Proxy(address(treasuryImpl), treasuryInitData)
        );
        console.log("Treasury Diamond deployed at:", treasuryProxy);

        // Deploy Facets
        console.log("2.3. Deploying Facets...");
        rewardDistributorFacet = address(new RewardDistributorFacet());
        assetManagerFacet = address(new AssetManagerAllocatorFacet());

        console.log("Reward Distributor Facet:", rewardDistributorFacet);
        console.log("Asset Manager Facet:", assetManagerFacet);

        // Deploy Asset Manager
        console.log("2.4. Deploying Asset Manager...");
        AssetManager assetManagerImpl = new AssetManager(
            usdcAddress,
            treasuryProxy
        );
        bytes memory assetManagerInitData = abi.encodeCall(
            AssetManager.initialize,
            (admin, governance)
        );
        assetManager = address(
            new ERC1967Proxy(address(assetManagerImpl), assetManagerInitData)
        );
        console.log("Asset Manager deployed at:", assetManager);
    ...
    function linkContracts() internal {
        ...
        assetManagerSelectors[0] = AssetManagerAllocatorFacet
            .netDeposits
            .selector;
        assetManagerSelectors[1] = AssetManagerAllocatorFacet
            .setAssetManager
            .selector;
        assetManagerSelectors[2] = AssetManagerAllocatorFacet
            .setAllocator
            .selector;
        assetManagerSelectors[3] = AssetManagerAllocatorFacet
            .transferUSDCtoAssetManager
            .selector;
        assetManagerSelectors[4] = AssetManagerAllocatorFacet
            .transferUSDCFromAssetManager
            .selector;
        assetManagerSelectors[5] = AssetManagerAllocatorFacet
            .transferUSDCForWithdrawal
            .selector;
        ...
        bytes4[] memory rewardDistributorSelectors = new bytes4[](6);
        rewardDistributorSelectors[0] = RewardDistributorFacet
            .successFee
            .selector;
        rewardDistributorSelectors[1] = RewardDistributorFacet
            .insuranceFund
            .selector;
        rewardDistributorSelectors[2] = RewardDistributorFacet
            .reportRewards
            .selector;
        rewardDistributorSelectors[3] = RewardDistributorFacet
            .setSuccessFeeFraction
            .selector;
        rewardDistributorSelectors[4] = RewardDistributorFacet
            .setInsuranceFundFraction
            .selector;
        rewardDistributorSelectors[5] = RewardDistributorFacet
            .setReporter
            .selector;

        treasury.addFacet(rewardDistributorFacet, rewardDistributorSelectors);
        console.log("RewardDistributorFacet added");

        // Link Asset Manager to Treasury
        console.log("3.3.4. Linking Asset Manager to Treasury...");
        AssetManagerAllocatorFacet(address(treasury)).setAssetManager(
            assetManager
        );
    ...
    function verifyDeployment() internal {
        ...
        USX usx = USX(usxProxy);
        require(
            keccak256(abi.encodePacked(usx.name())) ==
                keccak256(abi.encodePacked("USX")),
            "USX name verification failed"
        );
        require(
            keccak256(abi.encodePacked(usx.symbol())) ==
                keccak256(abi.encodePacked("USX")),
            "USX symbol verification failed"
        );
        require(usx.decimals() == 18, "USX decimals verification failed");
        require(
            address(usx.USDC()) == usdcAddress,
            "USX USDC address verification failed"
        );
        require(
            address(usx.treasury()) == treasuryProxy,
            "USX treasury verification failed"
        );
        ...
        StakedUSX susx = StakedUSX(susxProxy);
        require(
            keccak256(abi.encodePacked(susx.name())) ==
                keccak256(abi.encodePacked("sUSX")),
            "StakedUSX name verification failed"
        );
        require(
            keccak256(abi.encodePacked(susx.symbol())) ==
                keccak256(abi.encodePacked("sUSX")),
            "StakedUSX symbol verification failed"
        );
        require(
            susx.decimals() == 18,
            "StakedUSX decimals verification failed"
        );
        require(
            address(susx.USX()) == usxProxy,
            "StakedUSX USX address verification failed"
        );
        require(
            address(susx.treasury()) == treasuryProxy,
            "StakedUSX treasury verification failed"
        );
        ...
        TreasuryDiamond treasury = TreasuryDiamond(payable(treasuryProxy));
        require(
            address(treasury.USDC()) == usdcAddress,
            "Treasury USDC address verification failed"
        );
        require(
            address(treasury.USX()) == usxProxy,
            "Treasury USX address verification failed"
        );
        require(
            address(treasury.sUSX()) == susxProxy,
            "Treasury sUSX address verification failed"
        );
        require(
            treasury.governance() == governance,
            "Treasury governance verification failed"
        );
        require(
            treasury.assetManager() == assetManager,
            "Treasury asset manager verification failed"
        );
        console.log("Treasury verification passed");

        // Verify Facet Accessibility
        RewardDistributorFacet(treasuryProxy).successFee(1000000);
    ...
    function testBasicFunctionality() internal {
        ...
        require(
            susx.totalSupply() == 0,
            "StakedUSX initial supply should be 0"
        );
        ...
        TreasuryDiamond treasury = TreasuryDiamond(payable(treasuryProxy));
        require(
            treasury.successFeeFraction() == 50000,
            "Treasury successFeeFraction should be 50000"
        );
        require(
            treasury.insuranceFundFraction() == 50000,
            "Treasury insuranceFundFraction should be 50000"
        );
        console.log("Treasury basic functionality verified");

        // Test facet functionality through diamond
        uint256 successFee = RewardDistributorFacet(treasuryProxy).successFee(1000000);
        require(successFee == 50000, "successFee should be 50000 (5% of 1000000)");
