...
contract DeployScroll is Script {
    ...
    function deployCoreContracts() internal {
        ...
        bytes memory usxInitData = abi.encodeCall(USX.initialize, (
            usdcAddress,
        ...
        // Deploy proxy
        usxProxy = address(new ERC1967Proxy(
            address(usxImpl),
            usxInitData
        ));
        ...
        bytes memory susxInitData = abi.encodeCall(sUSX.initialize, (
            address(usxProxy),
            address(0), // Treasury address (will be set later)
            governance
        ));
        ...
        // Deploy proxy
        susxProxy = address(new ERC1967Proxy(
            address(susxImpl),
            susxInitData
        ));
    ...
    function deployDiamondWithFacets() internal {
        ...
        bytes memory treasuryInitData = abi.encodeCall(TreasuryDiamond.initialize, (
            usdcAddress,
            address(usxProxy),
            address(susxProxy),
            governance,
        ...
        treasuryProxy = address(new ERC1967Proxy(
    ...
    function verifyDeployment() internal {
        ...
        // Verify USX
        USX usx = USX(usxProxy);
        require(keccak256(abi.encodePacked(usx.name())) == keccak256(abi.encodePacked("USX Token")), "USX name verification failed");
        require(keccak256(abi.encodePacked(usx.symbol())) == keccak256(abi.encodePacked("USX")), "USX symbol verification failed");
        ...
        // Verify sUSX
        sUSX susx = sUSX(susxProxy);
        require(keccak256(abi.encodePacked(susx.name())) == keccak256(abi.encodePacked("sUSX Token")), "sUSX name verification failed");
        require(keccak256(abi.encodePacked(susx.symbol())) == keccak256(abi.encodePacked("sUSX")), "sUSX symbol verification failed");
    ...
    function testBasicFunctionality() internal {
        ...
        (bool success, bytes memory data) = treasuryProxy.call(abi.encodeWithSelector(AssetManagerAllocatorFacet.maxLeverage.selector));
        ...
        (success, data) = treasuryProxy.call(abi.encodeWithSelector(ProfitAndLossReporterFacet.successFee.selector, 1000000));
