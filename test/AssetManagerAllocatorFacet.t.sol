...
contract AssetManagerAllocatorFacetTest is DeployTestSetup {
    ...
    function test_maxLeverage_large_vault() public {
        ...
        assertTrue(usx.balanceOf(address(susx)) > 1000000e18, "Vault should have large USX balance from multiple deposits");
    }
    ...
    function test_checkMaxLeverage_within_limit() public {
        ...
        bytes memory data = abi.encodeWithSelector(
            AssetManagerAllocatorFacet.checkMaxLeverage.selector,
            depositAmount
        );
        (bool success, bytes memory result) = address(treasury).call(data);
    ...
    function test_transferUSDCFromAssetManager_success() public {
        ...
        bytes memory withdrawData = abi.encodeWithSelector(
            AssetManagerAllocatorFacet.transferUSDCFromAssetManager.selector,
            transferAmount
        );
    ...
    function test_complete_asset_manager_workflow() public {
        ...
        bytes memory setLeverageData = abi.encodeWithSelector(
            AssetManagerAllocatorFacet.setMaxLeverageFraction.selector,
            newLeverageFraction
        );
        ...
        bytes memory maxLeverageData = abi.encodeWithSelector(
            AssetManagerAllocatorFacet.maxLeverage.selector
        );
